...
class Ui_TransactionDataWindow(object):
    ...
    def email_sent_popup(self):
        popup2=QMessageBox()
        popup2.setWindowTitle("Sent!")
        popup2.setText("Transactions Sent Succesfully!")
        popup2.setIcon(QMessageBox.Information)
        popup2.setStandardButtons(QMessageBox.Ok)
        x=popup2.exec_()
    
    def ref(self):
        self.ChangingLabel.setText("Refreshing Transactions")
    ...
    def email_send(self):
        self.email_address="avixiontest@gmail.com"
        self.password="Techno123@"
        ...
        with smtplib.SMTP('smtp.gmail.com',587) as smtp:
            ...
            self.subject="Your Transaction Details"
            with open('C:/Program Files/accounts.json') as f:
                data=json.load(f)
            self.accountdetailsN=data['people'][self.num]
            self.body=self.accountdetails['log']
            self.reciever_email=self.accountdetailsN['email']
            self.msg=f'Subject: {self.subject}\n\n{self.body}'

            smtp.sendmail(self.email_address,self.reciever_email,self.msg)

        self.ChangingLabel.setText("Sending Transactions")
        self.ChangingLabel.setGeometry(QtCore.QRect(0, 490, 281, 61))
        self.EntryBar.setGeometry(QtCore.QRect(290, 510, 291, 31))
        self.step=0
        while self.step<=101:
    ...
    def setupUi(self, TransactionDataWindow):
        ...
        self.EmailBtn.setGeometry(QtCore.QRect(580, 100, 211, 61))
        font = QtGui.QFont()
        font.setFamily("Bite Chocolate")
        font.setPointSize(9)
        self.EmailBtn.setFont(font)
        self.EmailBtn.setObjectName("EmailBtn")
        self.EmailBtn.clicked.connect(self.email_send)
    ...
    def retranslateUi(self, TransactionDataWindow):
        ...
        self.EmailBtn.setText(_translate("TransactionDataWindow", "Email My Transactions"))
